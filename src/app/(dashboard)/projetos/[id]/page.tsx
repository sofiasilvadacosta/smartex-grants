...
export default async function ProjectDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const [unmatchedCount, unmatchedRhCount, divergences, receipts] = await Promise.all([
    ...
    <div>
      <div className="flex items-center justify-between">
        ...
        <div className="flex flex-wrap gap-2">
          ...
          <Link
            href={`/projetos/${project.id}/alocacao`}
            className="rounded-md border border-gray-300 px-4 py-2 text-sm font-medium text-gray-700 hover:bg-gray-50"
          >
            Alocação
          </Link>
          ...
          <Link
            href={`/projetos/${project.id}/recebimentos`}
            className="rounded-md border border-gray-300 px-4 py-2 text-sm font-medium text-gray-700 hover:bg-gray-50"
          >
            Recebimentos ({project._count.receipts})
          </Link>
          <Link
            href={`/projetos/${project.id}/deliverables`}
            className="rounded-md border border-gray-300 px-4 py-2 text-sm font-medium text-gray-700 hover:bg-gray-50"
          >
            Deliverables ({project._count.deliverables})
          </Link>
        </div>
      </div>
      ...
      <dl className="mt-6 grid grid-cols-2 gap-4 sm:grid-cols-5">
        ...
        <div className="rounded-lg border border-gray-200 bg-white p-4">
          ...
          <dd className="mt-1 text-lg font-semibold text-gray-900">{eur(receipts.receivedTotal)}</dd>
        </div>
        <div className="rounded-lg border border-gray-200 bg-white p-4">
          <dt className="text-xs text-gray-500">Aprovado por receber</dt>
          <dd
            className={`mt-1 text-lg font-semibold ${
              receipts.outstandingTotal > 0 ? "text-amber-700" : "text-gray-900"
            }`}
          >
            {eur(receipts.outstandingTotal)}
          </dd>
        </div>
      </dl>
